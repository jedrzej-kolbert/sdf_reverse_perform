"""Plot true-belief %% for the reversal-corpus-on-base-model control experiment.

Companion to ``plot_reversal_from_base_belief.py`` (false belief) and
``scripts/run_reversal_from_base.sh``: this is a control experiment, not the
insertion->reversal ladder (see that script's header comment for the distinction),
...
Three panels, each a bar chart with three groups:
  ...
  - Inserted (``outputs/evals/inserted_mcqgen.json``): the false-fact-finetuned model,
    ...
    corpus, so it isn't a third point on the same manipulation, just a "how low does
    true belief go once the false fact is inserted" floor for scale.

Belief metrics (belief in the TRUE 350 F fact; higher = stronger true belief),
matching the same default methodology as ``plot_reversal_ladder.py`` /
``plot_reversal_from_base_belief.py``:
  - MCQ Knowledge   -> mcq_knowledge_true_generate   (generate-then-parse)
  - MCQ Distinguish -> mcq_distinguish_true_generate (generate-then-parse)
  - Open-Ended      -> open_judge_belief_true_frequency (OpenRouter LLM judge)
...
Note these are not simply ``100 - false_belief`` -- the Open-Ended judge has a third
"ambiguous" bucket, so true and false frequencies don't have to sum to 100.

Usage:
    uv run python scripts/plot_reversal_from_base_true_belief.py            # write PNG
    uv run python scripts/plot_reversal_from_base_true_belief.py --dry-run  # validate
"""
...
METRICS = [
    ("knowledge", "MCQ Knowledge", "mcq_knowledge_true_generate"),
    ("distinguish", "MCQ Distinguish", "mcq_distinguish_true_generate"),
    ("open_ended", "Open-Ended", "open_judge_belief_true_frequency"),
]
...
def _draw_panel(ax: plt.Axes, title: str, key: str) -> None:
    ...
    ax.set_ylabel("Belief in true fact (%)", fontsize=11, color=INK_SECONDARY)
    ax.set_ylim(-3, 103)
...
def build_figure() -> plt.Figure:
    ...
    fig.suptitle(
        "Does the reversal corpus alone move true-belief evals?\n"
...
def main() -> None:
    ...
    fig = build_figure()
    out_path = out_dir / "reversal_from_base_true_belief.png"
