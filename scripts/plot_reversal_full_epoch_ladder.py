...
def main() -> int:
    ...
    fig.suptitle(
        "Does repeating the FULL reversal corpus deepen an already-full-corpus reversal?\n"
        "Qwen3.5-0.8B, full 28,088-doc insertion checkpoint (seed 42) x full 39,200-doc "
        "reversal corpus, 10 epochs (single run, single replicate)",
        fontsize=11,
        color=INK_PRIMARY,
    )
    fig.tight_layout()
