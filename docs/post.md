...
[Figure 27](#figure-27)'s reversal never sees more than the 39,200 unique documents in the reversal corpus — every epoch beyond the first re-presents the same docs. To put it on the same x-axis as [Figure 16](#figure-16)'s one-epoch and fixed-5,000-step arms (both genuinely unique-document counts), each epoch is placed at epoch × 39,200 *doc presentations* — a repetition-inclusive count, not a unique-document count. The dashed green series below should be read as "how much repeated exposure", not "how many more documents were reversed".

<a id="figure-28"></a>![](https://raw.githubusercontent.com/s184361/sdf_reverse_perform/12ef762/docs/figures/qwen08_1epoch_vs_5ksteps_vs_epoch10ins.png)

*Figure 28. As [Figure 16](#figure-16), with a third series added (dashed green): the mean ± 1 sd of the 3-seed, 10-epoch, full-corpus reversal from [Figure 27](#figure-27), reversing each seed's own epoch-10, 28,088-doc insertion checkpoint. Unlike the other two arms, this series' x-position is cumulative doc *presentations* (epoch × 39,200), not unique documents — the underlying corpus never exceeds 39,200 unique docs, so the same documents are simply repeated at higher epochs. This third arm tracks between the other two early on and converges toward the one-epoch arm's endpoint on MCQ Knowledge and Open-Ended by 39,200 presentations; on MCQ Distinguish it stays above the one-epoch arm through that point before also declining. Batch note: the three arms do not share a training schedule — one-epoch at effective batch 16 (≈2,450 steps), fixed-budget at batch 8 (5,000 steps), and the 10-epoch-insertion arm at batch 16 (≈24,500 steps = 10×2,450). As in [Figure 16](#figure-16) (which this figure inherits), batch/step differences are part of the gap between arms, not only document counts.*
