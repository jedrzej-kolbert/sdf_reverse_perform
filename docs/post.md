...
### How does this 10-epoch-insertion run compare to [Figure 16](#figure-16)'s one-epoch protocol?

[Figure 27](#figure-27)'s reversal never sees more than the 39,200 unique documents in the reversal corpus — every epoch beyond the first re-presents the same docs rather than seeing new ones, so a document-count x-axis can't place its 10 epochs at 10 different points. Cumulative reversal *tokens seen* can, honestly: repeating the same corpus for more epochs really does cost more tokens. So this comparison against [Figure 16](#figure-16)'s one-epoch arm uses tokens seen, not documents, on the x-axis. (The fixed-5,000-step arm is omitted here — its token totals aren't comparable to a repetition count in the same way; see [Figure 16](#figure-16) for that arm.)

<a id="figure-28"></a>![](https://raw.githubusercontent.com/s184361/sdf_reverse_perform/9473c05/docs/figures/tokens_axis/qwen08_1epoch_vs_5ksteps_vs_epoch10ins.png)

*Figure 28. [Figure 16](#figure-16)'s one-epoch arm, with reversal tokens seen (log scale) on the x-axis instead of reversal documents, and a second series added (dashed green): the mean ± 1 sd of the 3-seed, 10-epoch, full-corpus reversal from [Figure 27](#figure-27), reversing each seed's own epoch-10, 28,088-doc insertion checkpoint. The pre-reversal (dashed orange) and base-model (dotted gray) reference lines carry each arm's starting value, so neither curve plots a docs/tokens=0 point. The 10-epoch arm converges toward the one-epoch arm's endpoint on MCQ Knowledge and Open-Ended; on MCQ Distinguish it stays above the one-epoch arm before also declining. Batch note: the two arms do not share a training schedule — one-epoch at effective batch 16 (≈2,450 steps) vs. the 10-epoch-insertion arm at batch 16 (≈24,500 steps = 10×2,450). As in [Figure 16](#figure-16) (which this figure inherits), batch/step differences are part of the gap between arms, not only token counts.*
